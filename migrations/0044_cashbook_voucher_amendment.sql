...
DO $migration$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.schema_migrations WHERE version = '0043') THEN
    RAISE EXCEPTION 'Migration 0044 requires migration 0043';
  END IF;
  IF to_regprocedure('public.p13_classify_cashbook(text)') IS NULL
     OR to_regprocedure('public.p2_cashbook_response(text)') IS NULL THEN
    RAISE EXCEPTION 'Migration 0044 requires the cashbook compatibility functions';
  END IF;
END
$migration$;

-- A voucher's operational collector is not the immutable actor who created it.
-- Keep both identities so editing "Người thu/chi" never rewrites audit history.
...
UPDATE public.cashbook_transactions entry
SET collector_name = COALESCE(NULLIF(entry.collector_name, ''), NULLIF(entry.creator, ''), entry.created_by),
    counterparty_type = COALESCE(NULLIF(entry.counterparty_type, ''), CASE
      WHEN entry.customer_id IS NOT NULL THEN 'customer'
      WHEN entry.supplier_id IS NOT NULL THEN 'supplier'
      ELSE 'other'
    END),
    counterparty_id = COALESCE(NULLIF(entry.counterparty_id, ''), entry.customer_id, entry.supplier_id)
WHERE entry.collector_name IS NULL
   OR entry.counterparty_type IS NULL;

CREATE INDEX IF NOT EXISTS cashbook_collector_idx
  ON public.cashbook_transactions(collector_id, transaction_date DESC);
CREATE INDEX IF NOT EXISTS cashbook_counterparty_idx
  ON public.cashbook_transactions(counterparty_type, counterparty_id, transaction_date DESC);
...
DECLARE
  ...
  amendment_reason text := COALESCE(NULLIF(btrim(p_input->>'reason'), ''), 'Sửa phiếu thu/chi');
BEGIN
  ...
  IF lower(COALESCE(entry.status, '')) IN ('cancelled', 'canceled', 'đã hủy', 'da huy')
  ...
  next_category := COALESCE(NULLIF(btrim(p_input->>'category'), ''), entry.category, 'Khác');
  next_partner := COALESCE(NULLIF(btrim(p_input->>'partner'), ''), entry.partner, 'Khác');
...
VALUES ('0044', 'Amend receipt and payment vouchers with routed debt-safe adjustments')
