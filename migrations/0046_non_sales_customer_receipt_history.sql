...
-- Reconcile standalone customer receipts created after the earlier backfill,
-- including non-sales categories such as shipping support or penalties. Only a
-- voucher whose partner text uniquely and exactly identifies one active customer
-- is touched; unrelated standalone receipts remain unchanged.
...
CREATE TEMP TABLE p46_cashbook_reconciliation ON COMMIT DROP AS
...
SELECT
  ...
  left('reconcile:0046:' || sequenced.cashbook_id, 128) AS reconcile_key,
...
BEGIN
  IF EXISTS (
    SELECT 1
    FROM p46_cashbook_reconciliation repair
    JOIN public.payments payment
      ON payment.cashbook_transaction_id = repair.cashbook_id
  ) THEN
    RAISE EXCEPTION 'Migration 0046 stopped: a candidate voucher already has a payment row';
  END IF;
END
$validation$;
...
SELECT
  'PAY-RECON-0046-' || repair.cashbook_id,
  repair.customer_id,
  repair.amount,
  ...
  repair.request_hash,
  COALESCE(repair.created_by, 'migration:0046'),
  now()
FROM p46_cashbook_reconciliation repair
...
SELECT
  'DTX-RECON-0046-' || repair.cashbook_id,
  repair.customer_id,
  'payment',
  repair.amount,
  -repair.amount,
...
FROM (
  ...
  FROM p46_cashbook_reconciliation
...
VALUES ('0046', 'Reconcile later non-sales customer receipts into debt history')
