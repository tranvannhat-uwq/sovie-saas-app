...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.schema_migrations WHERE version = '0084') THEN
    RAISE EXCEPTION 'Migration 0085 requires migration 0084';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'saas_rpc_executor') THEN
    RAISE EXCEPTION 'Migration 0085 requires tenant RPC executor migration 0060';
  END IF;
END;
$prerequisite$;

-- Compact, read-only models for the Admin mobile application. List functions
-- intentionally avoid SELECT * and large order-item payloads. Detail payloads
-- are fetched only after the user opens a record.
  ...
  RETURN jsonb_build_object(
    ...
    'by_company', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        ...
        'amount', ranked.item->'amount'
      ) ORDER BY ranked.position)
      FROM (
        SELECT item, position
        FROM jsonb_array_elements(COALESCE(raw->'by_company', '[]'::jsonb))
          WITH ORDINALITY expanded(item, position)
        LIMIT 5
      ) ranked
      LEFT JOIN public.companies company ON company.id = ranked.item->>'key'
    ), '[]'::jsonb),
    'by_brand', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        'key', ranked.item->>'key',
        'name', COALESCE((
          SELECT brand.name FROM public.brands brand
          WHERE brand.id = ranked.item->>'key' OR brand.name = ranked.item->>'key'
          ORDER BY CASE WHEN brand.id = ranked.item->>'key' THEN 0 ELSE 1 END
          LIMIT 1
      ...
      FROM (
        SELECT item, position
        FROM jsonb_array_elements(COALESCE(raw->'by_brand', '[]'::jsonb))
          WITH ORDINALITY expanded(item, position)
        LIMIT 5
      ) ranked
    ), '[]'::jsonb),
    'by_salesperson', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        'key', ranked.item->>'key',
        'name', COALESCE((
          SELECT NULLIF(COALESCE(profile.display_name, profile.username), '')
          FROM public.profiles profile
          WHERE profile.id = ranked.item->>'key'
             OR profile.username = ranked.item->>'key'
             OR profile.auth_user_id::text = ranked.item->>'key'
          LIMIT 1
      ...
      FROM (
        SELECT item, position
        FROM jsonb_array_elements(COALESCE(raw->'by_salesperson', '[]'::jsonb))
          WITH ORDINALITY expanded(item, position)
        LIMIT 5
      ) ranked
    ), '[]'::jsonb),
    'by_customer', COALESCE((
      ...
      FROM (
        SELECT item, position
        FROM jsonb_array_elements(COALESCE(raw->'by_customer', '[]'::jsonb))
          WITH ORDINALITY expanded(item, position)
        LIMIT 5
      ) ranked
    ), '[]'::jsonb),
    'series', COALESCE((
      ...
      FROM (
        SELECT item
        FROM jsonb_array_elements(COALESCE(raw->'series', '[]'::jsonb)) item
    ...
    'top_skus', COALESCE((
      ...
      FROM (
        SELECT item, position
        FROM jsonb_array_elements(COALESCE(raw->'top_skus', '[]'::jsonb))
          WITH ORDINALITY expanded(item, position)
        LIMIT 5
      ) ranked
    ), '[]'::jsonb),
...
BEGIN
  ...
  WITH combined AS (
    ...
    UNION ALL
    SELECT draft.id, 'draft'::text source, draft.customer_id,
      draft.customer_name, COALESCE(customer.phone, '') customer_phone,
      COALESCE(draft.order_date, draft.created_at) order_date,
      'draft'::text status, COALESCE(draft.total_payable, 0) total_payable,
      COALESCE(draft.paid_amount, 0) paid_amount,
      jsonb_array_length(COALESCE(draft.items, '[]'::jsonb)) item_count,
      COALESCE((SELECT sum(COALESCE((item->>'quantity')::numeric, 0))
        FROM jsonb_array_elements(COALESCE(draft.items, '[]'::jsonb)) item), 0) quantity
    ...
    WHERE (search_term = '' OR row_data.id ILIKE '%' || search_term || '%'
      ...
      AND (status_filter = 'all'
        ...
        OR (status_filter <> 'draft' AND row_data.source = 'finalized' AND lower(row_data.status) = status_filter))
  ), page AS (
    SELECT * FROM filtered ORDER BY order_date DESC, id DESC
...
BEGIN
  ...
  IF lower(COALESCE(p_source, 'finalized')) = 'draft' THEN
    SELECT jsonb_build_object(
      'id', draft.id, 'source', 'draft', 'customer_id', draft.customer_id,
      'customer_name', draft.customer_name, 'customer_phone', COALESCE(customer.phone, ''),
      'order_date', COALESCE(draft.order_date, draft.created_at), 'status', 'draft',
      'total_payable', COALESCE(draft.total_payable, 0), 'paid_amount', COALESCE(draft.paid_amount, 0),
      'notes', COALESCE(draft.notes, ''), 'items', COALESCE(draft.items, '[]'::jsonb)
    ) INTO result
    FROM public.draft_orders draft
    LEFT JOIN public.customers customer ON customer.id = draft.customer_id
    WHERE draft.id = p_id;
  ELSE
    SELECT jsonb_build_object(
      'id', order_row.id, 'source', 'finalized', 'customer_id', order_row.customer_id,
      'customer_name', order_row.customer_name, 'customer_phone', COALESCE(customer.phone, ''),
      ...
      'notes', COALESCE(order_row.notes, ''), 'items', COALESCE(order_row.items, '[]'::jsonb)
    ) INTO result
    FROM public.orders order_row
    LEFT JOIN public.customers customer ON customer.id = order_row.customer_id
  ...
  WITH filtered AS (
    SELECT customer.id, customer.code, customer.name, COALESCE(customer.phone, '') phone,
      ...
      COALESCE((SELECT NULLIF(COALESCE(profile.display_name, profile.username), '')
        FROM public.profiles profile
        WHERE profile.id = customer.managed_by OR profile.username = customer.managed_by
          OR profile.auth_user_id::text = customer.managed_by LIMIT 1), customer.managed_by, '') managed_by
    ...
    SELECT * FROM filtered ORDER BY last_order_at DESC NULLS LAST, name
    LIMIT safe_limit OFFSET safe_offset
  )
  SELECT jsonb_build_object(
    'total', (SELECT count(*) FROM filtered), 'limit', safe_limit, 'offset', safe_offset,
    'data', COALESCE((SELECT jsonb_agg(to_jsonb(page) ORDER BY last_order_at DESC NULLS LAST, name) FROM page), '[]'::jsonb)
  ) INTO result;
  RETURN result;
END;
$$;
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'managed_by', COALESCE((SELECT NULLIF(COALESCE(profile.display_name, profile.username), '')
      FROM public.profiles profile
      WHERE profile.id = customer.managed_by OR profile.username = customer.managed_by
        OR profile.auth_user_id::text = customer.managed_by LIMIT 1), customer.managed_by, ''),
    'debt', COALESCE(customer.debt, 0), 'total_transaction', COALESCE(customer.total_transaction, 0),
  ...
  ) INTO result FROM public.customers customer
  WHERE customer.id = p_id AND customer.deleted_at IS NULL;
...
BEGIN
  ...
  WITH filtered AS (
    ...
    FROM public.customers customer
    WHERE customer.deleted_at IS NULL AND COALESCE(customer.debt, 0) <> 0
      AND (search_term = '' OR customer.code ILIKE '%' || search_term || '%'
        OR customer.name ILIKE '%' || search_term || '%' OR customer.phone ILIKE '%' || search_term || '%')
  ), page AS (
    SELECT * FROM filtered ORDER BY debt DESC, name LIMIT safe_limit OFFSET safe_offset
  )
  SELECT jsonb_build_object(
    'summary', jsonb_build_object(
...
BEGIN
  ...
  WITH filtered AS (
    ...
    SELECT * FROM filtered ORDER BY transaction_date DESC, id DESC LIMIT safe_limit OFFSET safe_offset
  )
  SELECT jsonb_build_object(
    'total', (SELECT count(*) FROM filtered), 'limit', safe_limit, 'offset', safe_offset,
    'data', COALESCE((SELECT jsonb_agg(to_jsonb(page) ORDER BY transaction_date DESC, id DESC) FROM page), '[]'::jsonb)
  ) INTO result;
  RETURN result;
END;
$$;
...
BEGIN
  ...
  WITH filtered AS (
    SELECT profile.id, profile.username, profile.display_name, profile.role,
      ...
      COALESCE(metrics.order_count, 0) order_count,
      COALESCE(metrics.net_sales, 0) net_sales
    FROM public.profiles profile
    LEFT JOIN LATERAL (
      SELECT
        (SELECT count(*) FROM public.customers customer
          WHERE customer.deleted_at IS NULL AND customer.managed_by IN (profile.id, profile.username, profile.auth_user_id::text)) customer_count,
        (SELECT sum(COALESCE(customer.debt, 0)) FROM public.customers customer
          WHERE customer.deleted_at IS NULL AND customer.managed_by IN (profile.id, profile.username, profile.auth_user_id::text)) current_debt,
        (SELECT count(*) FROM public.orders order_row
          LEFT JOIN public.customers customer ON customer.id = order_row.customer_id
    ...
    WHERE search_term = '' OR profile.username ILIKE '%' || search_term || '%'
      OR profile.display_name ILIKE '%' || search_term || '%'
      OR COALESCE(profile.position, '') ILIKE '%' || search_term || '%'
  ), page AS (
    SELECT * FROM filtered ORDER BY is_active DESC, net_sales DESC, display_name
    LIMIT safe_limit OFFSET safe_offset
  )
  SELECT jsonb_build_object(
    'total', (SELECT count(*) FROM filtered), 'limit', safe_limit, 'offset', safe_offset,
    'period', jsonb_build_object('start', p_start, 'end', p_end),
    'data', COALESCE((SELECT jsonb_agg(to_jsonb(page) ORDER BY is_active DESC, net_sales DESC, display_name) FROM page), '[]'::jsonb)
  ) INTO result;
  RETURN result;
END;
$$;
...
-- These two functions are SECURITY DEFINER. The non-BYPASSRLS executor created
-- by migration 0060 keeps every table read inside the active organization.
...
VALUES ('0085', 'Add compact paginated mobile Admin read models')
