...
DO $prerequisite$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.schema_migrations WHERE version = '0098') THEN
    RAISE EXCEPTION 'Migration 0099 requires migration 0098';
  END IF;
END;
$prerequisite$;

-- Keep every dashboard summary aligned with the selected customer and
-- salesperson while preserving the existing tenant-scoped reporting model.
...
VALUES ('0099', 'Apply customer and salesperson filters consistently across dashboard summaries')
