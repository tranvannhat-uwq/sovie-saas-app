...
test('SoVie is the primary project brand in navigation and login', () => {
  ...
  assert.match(html, /class="login-story-brand"[\s\S]*alt="SoVie"/);
  assert.match(html, /class="landing-brand"[\s\S]*alt="SoVie"/);
...
test('project logo presentation is prominent and responsive', () => {
  ...
  assert.match(css, /\.login-brand-lockup\s*\{[\s\S]*width:\s*118px;/);
